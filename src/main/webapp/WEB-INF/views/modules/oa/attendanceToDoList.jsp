	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${list}" var="att">
				<c:set var="task" value="${att.act.task}" />
				<c:set var="vars" value="${att.act.vars}" />
				<c:set var="procDef" value="${att.act.procDef}" />
				<c:set var="status" value="${att.act.status}" />
				<tr>
					<td>
					...
					<td>${procDef.name}</td>
					<td><b title='流程版本号'>V: ${att.procDef.version}</b></td>
					<td><fmt:formatDate value="${att.task.createTime}" type="both"/></td>
					<td>
						...
						<a target="_blank" href="${pageContext.request.contextPath}/act/diagram-viewer?processDefinitionId=${att.task.processDefinitionId}&processInstanceId=${att.task.processInstanceId}">跟踪</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>

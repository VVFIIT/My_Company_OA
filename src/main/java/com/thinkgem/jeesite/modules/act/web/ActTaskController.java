...
public class ActTaskController extends BaseController {
	...
	public String form(Act act, String resourceName, HttpServletRequest request, Model model) throws UnsupportedEncodingException {
	 ...
	 *                       imageStream =
	 *                       repositoryService.getResourceAsStream(procDef.
	 *                       getDeploymentId(), diagramResourceName); byte[] b =
	 *                       new byte[1024]; int len = -1; while ((len =
	 *                       imageStream.read(b, 0, 1024)) != -1) {
	 *                       response.getOutputStream().write(b, 0, len); } }
	 */
	 ...
	 *                       (procDefId) .singleResult(); ProcessDefinitionImpl
	 *                       pdImpl = (ProcessDefinitionImpl) processDefinition;
	 ...
	 *                       def.getActivities();// 获得当前任务的所有节点 List
	 ...
	 *                       (activityImpl.getActivities().size() > 1) { List
	 *                       <ActivityImpl> subAcList = activityImpl
